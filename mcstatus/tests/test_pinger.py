...
class TestServerPinger(IsolatedAsyncioTestCase):
    def setUp(self):
        self.pinger = ServerPinger(AsyncConnection(), host="localhost", port=25565, version=44)

    async def test_handshake(self):
        await self.pinger.handshake()
    ...
    async def test_read_status(self):
        ...
        status = await self.pinger.read_status()
    ...
    async def test_read_status_invalid_json(self):
        self.pinger.connection.receive(bytearray.fromhex("0300017B"))
        with self.assertRaises(IOError):
            await self.pinger.read_status()

    async def test_read_status_invalid_reply(self):

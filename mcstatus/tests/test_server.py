...
class TestMinecraftServer(IsolatedAsyncioTestCase):
    def setUp(self):
    ...
    async def test_ping(self):
        ...
        with patch("mcstatus.server.TCPSocketConnection.connect") as connection:
            connection.return_value = self.socket
            latency = await self.server.ping(ping_token=29704774, version=47)
    ...
    async def test_ping_retry(self):
        with patch("mcstatus.server.TCPSocketConnection.connect") as connection:
    ...
    async def test_status(self):
        ...
        with patch("mcstatus.server.TCPSocketConnection.connect") as connection:
            connection.return_value = self.socket
            info = await self.server.status(ping_token=29704774, version=47)
    ...
    async def test_status_retry(self):
        with patch("mcstatus.server.TCPSocketConnection.connect") as connection:
            connection.return_value = None
            with patch("mcstatus.server.ServerPinger") as pinger:
                pinger.side_effect = [Exception, Exception, Exception]
    ...
    async def test_query(self):
        ...
        with patch("mcstatus.server.UDPSocketConnection.connect") as connection:
            connection.return_value = self.socket
            info = await self.server.query()
    ...
    async def test_query_retry(self):
        with patch("mcstatus.server.UDPSocketConnection.connect") as connection:
            connection.return_value = None
